...
if (isset($_POST['update_profile'])) {
   ...
   if(empty($update_name) || empty($update_email)){
      ...
      mysqli_query($conn, "UPDATE `tbladmin` SET name = '$update_name', email = '$update_email' WHERE id = '$user_id'") or die('query failed');
   }
   ...
   if (!empty($update_image)) {
      if ($update_image_size > 2000000) {
         ...
         $image_update_query = mysqli_query($conn, "UPDATE `tbladmin` SET image = '$update_image' WHERE id = '$user_id'") or die('query failed');
...
<body>
   ...
   <div class="update-profile">
      ...
      <?php
      $select = mysqli_query($conn, "SELECT * FROM `tbladmin` WHERE id = '$user_id'") or die('query failed');
